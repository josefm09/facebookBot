...
* Almacena archivos.
*
* Retorna el id del archivo almacenado
*
*/
...
function almacenar_archivo($configuraciones_sistema, $s3Client, $mysqli, $filename, $hash, $ext, $tipo_almacenado, $estatus)
  {
  /*
  ...
  */
  ...
  $ruta_almacenado_local = $configuraciones_sistema['directorio_local_archivos'];
  ...
  */
  if($tipo_almacenado === "local")
    ...
    //Inicio del query preparado
		$stmt = $mysqli->prepare("INSERT INTO `archivo` (`id_archivo`, `nombre_original`, `nombre_de_almacenado`, `extencion`,`tipo_almacenado`, `estatus`, `fecha_creacion`, `ultima_modificacion`)
														VALUES
														(NULL,?,?,?,?,?,now(),now())");
...
function consultar_archivo($configuraciones_sistema, $s3Client, $mysqli, $id_archivo)
  {
  $respuesta = array();//El valor que sera retornado en forma de json

  //******************** Verifica que la imagen exista ********************
	$query = "SELECT COUNT(*) FROM archivo WHERE `id_archivo` = ? AND `estatus` = 1";
	if ($stmt = $mysqli->prepare($query))
		...
		$stmt->bind_result($archivo_existe);
  ...
  /*
  * Si el id del archivo es encontrado en base de datos y como un valor
  * activo se procede a obtener la ruta de almacenado o el archivo en si
  * segun sea tu tipo de almacenado
  */
  if($archivo_existe == 1)
    {
		$query = "SELECT `nombre_original`, `extencion`, `nombre_de_almacenado`, `tipo_almacenado` FROM archivo WHERE `id_archivo` = ?";
		if ($stmt = $mysqli->prepare($query))
			{
			//Asigna las variables para el query
			$stmt->bind_param("i", $id_archivo);
    ...
    */
    ...
    if($tipo_almacenado == 's3_private')
      ...
      */
      ...
      /*
      * El archivo sera descargadO y guardado de manera temporal en esta
      * ruta
      */
